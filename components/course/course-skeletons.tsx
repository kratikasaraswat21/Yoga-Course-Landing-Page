...
export function CourseDetailSkeleton() {
  return (
    <div className="course-detail-page course-detail-skeleton" aria-label="Loading course details" role="status">
      ...
      <section className="course-detail-hero skeleton-hero">
        ...
        <div className="skeleton-detail-summary">
          <div className="skeleton-title-row">
            ...
            <Skeleton className="skeleton-share" />
          </div>
      ...
      <section className="course-video-section skeleton-video-section">
        <Skeleton className="skeleton-heading" />
        <Skeleton className="skeleton-subheading" />
        <div className="skeleton-video-grid">
          {[1, 2, 3, 4].map((item) => (
            <div className="skeleton-video-card" key={item}>
              <Skeleton className="skeleton-video-image" />
...
export function CourseListingSkeleton() {
  return (
    <div className="courses-page course-listing-skeleton" aria-label="Loading courses" role="status">
      <section className="courses-hero">
        <div>
          <Skeleton className="listing-eyebrow" />
          <Skeleton className="listing-title" />
          <Skeleton className="listing-copy" />
        </div>
        <Skeleton className="listing-hero-art" />
      </section>
      <Skeleton className="listing-search" />
      <div className="catalog-grid">
        {[1, 2, 3].map((item) => (
          <div className="catalog-card listing-card-skeleton" key={item}>
            <Skeleton className="listing-card-image" />
            <div className="listing-card-copy">
              <Skeleton className="listing-meta" />
              <Skeleton className="listing-card-title" />
              <Skeleton className="listing-card-description" />
              <Skeleton className="listing-card-price" />
            </div>
          </div>
        ))}
      </div>
...
export function VideoPlaybackSkeleton() {
  return (
    <div className="playback-skeleton" aria-label="Loading video playback" role="status">
      <Skeleton className="skeleton-breadcrumb" />
      <div className="playback-layout">
        <main className="playback-main">
          <Skeleton className="skeleton-player" />
          <Skeleton className="skeleton-lesson-label" />
          <Skeleton className="skeleton-playback-title" />
          <Skeleton className="skeleton-playback-copy" />
        ...
        <aside className="playback-sidebar skeleton-playback-sidebar">
          ...
          {[1, 2, 3, 4, 5, 6].map((item) => (
            <Skeleton className="skeleton-lesson-row" key={item} />
          ))}
        </aside>
      </div>
    </div>
