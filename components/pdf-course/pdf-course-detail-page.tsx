...
import type { PdfAccessResponse, PdfCourse, PdfCourseResponse, PdfPaymentVerifyResponse, PdfPurchaseOrderResponse } from "@/types/pdf-course";
...
export default function PdfCourseDetailPage() {
  ...
  const fetchCourse = useDebounce(async () => {
    try {
      setIsLoading(true);
      const response = await multipleApiHandler([{ endPoint: `/pdf-courses/${courseId}`, method: "GET", protected: true }]);
      const result = response[0]?.data as PdfCourseResponse | undefined;
      if (!response[0]?.ok || !result?.success || !result.data?.pdfCourse) throw new Error(result?.message ?? "PDF course could not be loaded.");
  ...
  const downloadPdf = useDebounce(async () => {
    if (!course) return;
    const response = await multipleApiHandler([{ endPoint: `/pdf-courses/${course.id}/access`, method: "GET", protected: true }]);
    const result = response[0]?.data as PdfAccessResponse | undefined;
    if (!response[0]?.ok || !result?.success || !result.data?.fileUrl) {
      toast.add({ title: "PDF could not be opened", description: result?.message ?? "Access could not be granted.", type: "error" });
      return;
    }
  ...
  const verifyPayment = useDebounce(async (payment: RazorpayPaymentResponse) => {
    ...
    if (!response[0]?.ok || !result?.success) {
      ...
      toast.add({ title: "Payment verification failed", description: result?.message ?? "Please try again.", type: "error" });
      return;
    }
    setPaymentStatus("idle");
    setCourse((currentCourse) => currentCourse ? { ...currentCourse, isPurchased: true, fileUrl: result.data?.fileUrl ?? null } : currentCourse);
  ...
  const purchaseCourse = useDebounce(async () => {
    ...
    if (!response[0]?.ok || !result?.success || !order?.orderId) {
      setPaymentStatus("idle");
      toast.add({ title: "Purchase could not be started", description: result?.message ?? "Please try again.", type: "error" });
      return;
    }
  ...
  return (
    <>
      <Script src="https://checkout.razorpay.com/v1/checkout.js" strategy="afterInteractive" onReady={() => setIsRazorpayReady(true)} />
      <div className="pdf-course-detail-page">
        <Link className="pdf-back-link" href="/pdf-course/explore"><ArrowLeft size={16} /> PDF courses</Link>
        <section className="pdf-course-detail-card">
          <div className="pdf-detail-cover"><Image src={course.thumbnailUrl || fallbackImage} alt={course.title} fill sizes="(max-width: 800px) 100vw, 48vw" /></div>
          <div className="pdf-detail-copy">
            <span className="eyebrow">PDF COURSE</span>
            <h1 className="title-case">{course.title}</h1>
            <p>{course.description}</p>
            {!course.isPurchased && !course.isAvailableForFree && <div className="pdf-detail-price"><strong>₹{(course.totalPayableAmount ?? course.price ?? 0).toLocaleString("en-IN")}</strong>{(course.discount ?? 0) > 0 && <del>₹{(course.price ?? 0).toLocaleString("en-IN")}</del>}</div>}
            <button className="detail-primary" disabled={paymentStatus !== "idle"} onClick={() => purchaseCourse()}>
              {paymentStatus === "opening" ? "Preparing payment..." : paymentStatus === "verifying" ? "Verifying payment..." : course.isPurchased ? "Download PDF" : course.isAvailableForFree ? "Get PDF" : "Purchase PDF"}
            </button>
            <div className="secure-payment"><ShieldCheck size={16} /> Secure access to your PDF</div>
          </div>
        </section>
        {course.isPurchased && <div className="pdf-access-note"><BookOpen size={18} /> Your access is active. The secure file link is generated when you open the PDF.</div>}
      </div>
    </>
  );
}
