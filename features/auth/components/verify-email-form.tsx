...
export function VerifyEmailForm({ email }: { email: string }) {
  ...
  const submitVerify = useDebounce(async (formValues: { email: string; otp: string }) => {
    try {
      ...
      if (!res?.data?.success) {
        toast.add({ title: "Email verification failed", description: res?.data?.message ?? "Please try again.", type: "error" })
        return
      }
  ...
  const submitResend = useDebounce(async (formEmail: string) => {
    try {
      ...
      setResendSeconds(60)
      toast.add({ title: "Verification code sent", description: "Check your email for the new code.", type: "success" })
    } catch {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!email) {
      toast.add({ title: "Email required", description: "We need your email address to verify this code.", type: "error" })
      return
    }
